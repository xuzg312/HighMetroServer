...
public class CamRemoteLinkImpl
{
        ...
        RealDataCallBack realDataCallBack,PlayCtrl.DeccbFun decodeCallback)
    {
        ...
        // 开启预览，传入码流回调
        _playHandle = HikSdk.NET_DVR_RealPlay_V40(_userId, ref playInfo, realDataCallBack, nint.Zero);
        if (_playHandle < 0)
            return GetLastError();
        if (PlayCtrl.PlayM4_Play(_iPort, nint.Zero)<=0) //传 IntPtr.Zero 表示软解码，触发回调
            return GetLastError();
