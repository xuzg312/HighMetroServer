                    ...
                    socketDataBlock.Content[1] == 0XAA)
            {
                ...
                switch (function)
                {
                    ...
                    case 0X06://自动
                        ...
                        //协议中去掉hostid
                        byte[] data = new byte[socketDataBlock.Length-2];
                        ...
                        byte length = socketDataBlock.Content[2];
                        socketDataBlock.Content[2] = (byte)(length - 2);
                        socketDataBlock.Length = data.Length;
                        break;
                    case 0XEC: //验证,实时监控数据；
                        tcpDataBean = new TcpDataBean();
                        tcpDataBean.TurnComm = false;
                        tcpDataBean.Type = PublicConst.IdentifyAll;
                        //hostbh；
                        iPosition = 3;
                        tcpDataBean.HostBh = publicUntil.GetUshort(socketDataBlock.Content, iPosition);
                        break;
                    case 0XED: //验证,仅发送心跳；
                        tcpDataBean = new TcpDataBean();
                        tcpDataBean.TurnComm = false;
                        tcpDataBean.Type = PublicConst.IdentifyHeart;
                        //hostbh；
                        iPosition = 3;
                        tcpDataBean.HostBh = publicUntil.GetUshort(socketDataBlock.Content, iPosition);
                        break;
                    case 0XEF: //客户端获取拍照的图片；
                        tcpDataBean = new TcpDataBean();
                        tcpDataBean.TurnComm = false;
                        tcpDataBean.Type = PublicConst.IdentifyPhoto;
                        //长度，1字节；
